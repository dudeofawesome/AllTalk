...
var apiKey = 'AIzaSyD2sUg38HszaeoUDeEsPx8fkAgXcn53tIQ';
var gcm = new GCM(apiKey);

var databaseUrl = "mongodb://localhost:27017/AllTalk"; // "username:password@example.com/mydb"
...
io.on('connection', function (socket) {
	...
	socket.on('get friends', function(msg) {
	...
	socket.on('get users', function(msg) {
		console.log("get auction item");
		db.items.find({_id: msg._id}, function(err, item) {
			if( err || !item || item.length == 0) io.to(socket.id).emit('get auction item', "Bad _id.");
			else {
				io.to(socket.id).emit('get auction item', item);
			}
		});
	});
